...
output_file="${data_type}_aggregators.pdf"
...
gnuplot <<EOF
    ...
    set xlabel "#aggregators" font "Times_New_Roman, 24" offset 0, -0.5
    ...
    set ytics 10 font "Times_New_Roman, 24"
    ...
    plot "${input_file}" every ::1::7 using (\$2/1000):xtic(1) title "CFNAgg" with histogram, \
        #  "${input_file}" every ::1::7 using (\$3/1000):xtic(1) title "CFNAgg 0%" with histogram, \
        #  "${input_file}" every ::1::7 using (\$4/1000):xtic(1) title "TCPAgg 0.5%" with histogram fill pattern 4, \
        #  "${input_file}" every ::1::7 using (\$5/1000):xtic(1) title "CFNAgg 0.5%" with histogram fill pattern 4
EOF
